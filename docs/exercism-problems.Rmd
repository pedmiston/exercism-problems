...
set2_palette <- make_color_palette(RColorBrewer::brewer.pal(3, "Set2"), c("green", "orange", "blue"))
```
...
ggplot(problems_by_language) +
  ...
  geom_smooth(aes(group = language), data = filter(problems_by_language, n_test_cases != 0, n_test_cases < 30),
              method = "lm", se = FALSE, size = 0.2, alpha = 0.2, color = set2_palette("blue")) +
...
ggplot(lang_pred_lines) +
  ...
  scale_shape_manual(values = c(4, 19)) +
  scale_color_manual(values = c("gray", set2_palette("blue"))) +
  theme(legend.position = "none")
